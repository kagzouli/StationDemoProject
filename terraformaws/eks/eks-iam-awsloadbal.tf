data "aws_iam_policy_document" "eks_alb_webidentity_role_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:${var.shared_namespace}:aws-load-balancer-controller-sa"]
    }
...
resource "aws_iam_role" "eks_alb_iam_role" {
  name               = "station-alb-iam-role"
  assume_role_policy = data.aws_iam_policy_document.eks_alb_webidentity_role_policy.json
  tags = {
    Name        = "station-alb-iam-role"
...
# Policy version 2.2.0 de ALB Load balancer controller - Doc officiel AWS
resource "aws_iam_policy" "eks_alb_iam_policy" {
  name = "station-alb-iam-policy"
  policy = <<POLICY
{
    "Version": "2012-10-17",
    "Statement": [
